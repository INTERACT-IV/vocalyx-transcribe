...
def transcribe_audio_task(self, transcription_id: str, use_distributed: bool = None):
    ...
    # Router vers le bon mode selon use_distributed
    if use_distributed is True:
        # MODE DISTRIBUÉ FORCÉ
        logger.info(
            f"[{transcription_id}] 🎯 ROUTING TO DISTRIBUTED MODE | "
            f"use_distributed=True (forced) | "
            f"Worker: {config.instance_name}"
        )
        return _transcribe_distributed_mode(self, transcription_id, file_path, api_client)
    elif use_distributed is False:
        # MODE CLASSIQUE FORCÉ
        logger.info(
            f"[{transcription_id}] 🎯 ROUTING TO CLASSIC MODE | "
            f"use_distributed=False (forced) | "
            f"Worker: {config.instance_name}"
        )
        return _transcribe_classic_mode(self, transcription_id, file_path, transcription, api_client)
    else:
        # use_distributed is None - décision automatique
        if use_distributed and file_path:
            file_path_obj = Path(file_path)
            if file_path_obj.exists():
                logger.info(
                    f"[{transcription_id}] 🎯 ROUTING TO DISTRIBUTED MODE | "
                    f"use_distributed=None but auto-decision=True | "
                    f"Worker: {config.instance_name}"
                )
                return _transcribe_distributed_mode(self, transcription_id, file_path, api_client)
        ...
        logger.info(
            f"[{transcription_id}] 🎯 ROUTING TO CLASSIC MODE | "
            f"use_distributed=None but auto-decision=False | "
            f"Worker: {config.instance_name}"
        )
        return _transcribe_classic_mode(self, transcription_id, file_path, transcription, api_client)


def _transcribe_distributed_mode(self, transcription_id: str, file_path: str, api_client):
    """
    Mode distribué : délègue à orchestrate_distributed_transcription
    """
    logger.info(
        f"[{transcription_id}] 🚀 DISTRIBUTED MODE FUNCTION CALLED | "
...
def _transcribe_classic_mode(self, transcription_id: str, file_path: str, transcription: dict, api_client):
    """
    Mode classique : traitement direct sur un seul worker
    """
    logger.info(
        f"[{transcription_id}] 🎯 CLASSIC MODE FUNCTION CALLED | "
        f"Worker: {config.instance_name} | "
